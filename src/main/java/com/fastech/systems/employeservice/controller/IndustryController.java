...
public class IndustryController {
    ...
    public ResponseEntity<?> getAllData(@RequestBody PaginationDto paginationDto) {
        APIResponse<Industry> responseModel = new APIResponse<>();
        try {
            Page<Industry> list = service.findAll(paginationDto);
    ...
    @GetMapping("findAllIndustriesByCompanyID")
    public ResponseEntity<?> findAllIndustriesByCompanyID(@RequestBody PaginationDto paginationDto) {
        APIResponse<Industry> responseModel = new APIResponse<>();
        try {
            Page<Industry> list = companyIndustryService.findAllIndustriesByCompanyID(paginationDto);
            responseModel.setResponse(list.getContent());
            responseModel.setTotalCount((int) list.getTotalElements());
            responseModel.setTotalPages(list.getTotalPages());
            return ResponseEntity.ok(responseModel);
    ...
    public ResponseEntity<?> findById(@RequestParam(name="id") Integer id) {
        APIResponse<Industry> responseModel = new APIResponse<>();
        try {
            List<Industry> list = Collections.singletonList(service.findById(id));
    ...
    public ResponseEntity<?> delete(@RequestParam(name="id") Integer id) {
        APIResponse<Industry> responseModel = new APIResponse<>();
        try {
            List<Industry> list = Collections.singletonList(service.removeUser(id));
            responseModel.setResponse(list);
            responseModel.setTotalCount((int) list.size());
            responseModel.setTotalPages(1);
            return ResponseEntity.ok(responseModel);
    ...
    @PostMapping("create")
    public ResponseEntity<?> addUser(@RequestBody IndustryDto dto) {
        APIResponse<Industry> responseModel = new APIResponse<>();
        try {
            Industry saved = service.create(dto);
    ...
    @PutMapping("update")
    public ResponseEntity<?> update(@RequestBody IndustryDto dto) {
        APIResponse<Industry> responseModel = new APIResponse<>();
        try {
            List<Industry> list = Collections.singletonList(service.update(dto));
            responseModel.setResponse(list);
            responseModel.setTotalCount((int) list.size());
            responseModel.setTotalPages(1);
            return ResponseEntity.ok(responseModel);
    ...
    public static ResponseEntity<APIResponse<Industry>> errorResponse(APIResponse<Industry> responseModel, Exception e) {
        log.error(e.getMessage());
