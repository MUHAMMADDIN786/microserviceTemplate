...
public class ProjectController {
    ...
    public ResponseEntity<?> findById(@RequestParam(name="id") Integer id) {
        APIResponse<Project> responseModel = new APIResponse<>();
        try {
            List<Project> list = Collections.singletonList(service.findById(id));
    ...
    public ResponseEntity<?> delete(@RequestParam(name="id") Integer id) {
        APIResponse<Project> responseModel = new APIResponse<>();
        try {
            List<Project> list = Collections.singletonList(service.removeUser(id));
            responseModel.setResponse(list);
            responseModel.setTotalCount((int) list.size());
            responseModel.setTotalPages(1);
            return ResponseEntity.ok(responseModel);
    ...
    @PostMapping("create")
    public ResponseEntity<?> addUser(@RequestBody ProjectDto dto) {
        APIResponse<Project> responseModel = new APIResponse<>();
        try {
            Project saved = service.create(dto);
    ...
    @PutMapping("update")
    public ResponseEntity<?> update(@RequestBody ProjectDto dto) {
        APIResponse<Project> responseModel = new APIResponse<>();
        try {
            List<Project> list = Collections.singletonList(service.update(dto));
            responseModel.setResponse(list);
            responseModel.setTotalCount((int) list.size());
            responseModel.setTotalPages(1);
            return ResponseEntity.ok(responseModel);
    ...
    public static ResponseEntity<APIResponse<Project>> errorResponse(APIResponse<Project> responseModel, Exception e) {
        log.error(e.getMessage());
